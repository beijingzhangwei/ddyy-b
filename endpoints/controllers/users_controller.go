...
func (server *Server) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	user.UserID = uint64(time.Now().Unix())
	...
	fmt.Println("CreateUser.SaveUser before：", user)
...
func (server *Server) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Println("UpdateUser.UpdateAUser err：", err)
		formattedError := formaterror.FormatError(err.Error())
		responses.ERROR(w, http.StatusInternalServerError, formattedError)
		return
	}
...
func (server *Server) DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	tokenID, err := auth.ExtractTokenID(r)
	if err != nil {
		responses.ERROR(w, http.StatusUnauthorized, errors.New("ExtractTokenID error --> Unauthorized"))
		return
	}
	if tokenID != 0 && tokenID != uid {
		fmt.Println(" Unauthorized error for: tokenID", tokenID, "uid", uid)
		responses.ERROR(w, http.StatusUnauthorized, errors.New(http.StatusText(http.StatusUnauthorized)))
		return
	}
	_, err = user.DeleteAUser(server.DB, uid)
	if err != nil {
		fmt.Println(" DeleteAUser error for: tokenID", tokenID, "uid", uid)
		responses.ERROR(w, http.StatusInternalServerError, err)
		return
	}
